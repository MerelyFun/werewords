...
## 2026-09-09：Steam 三组前三

- 按用户要求核验订阅最高、最新发布且订阅>100两组，补充最近更新且订阅>10000一组；9项来源与扫描边界记录在docs/WORD_SOURCES.md。
- 新发布前三中最新为2026-01-31，过万更新前三最新为2024-04-13。统一使用北京时间，区分创建和更新字段，辨别462词与已导入243词为不同工坊条目。
- 本轮仅追加调查文档，保留已有代码改动；未下载或导入新增词库、未部署，未运行代码测试。下一步按用户选择继续。
...
## 2026-09-09：白天单屏游戏台完成

- 天亮、白天猜词、指认、讨论、人工结算与结果页移除反向面板；夜晚保持双向，镇长选词保持单向。音频阶段判定未改。
- 居中原创人物插图、大号计时、进度细线与回答选中态，独立 day-stage.css 隔离封面；横屏恢复单列，避免原双列样式留下半屏空白。
...
- 功能提交612a384；首次CI的全词库来源测试因逐词全表扫描超过5秒，fd5ebda改用ID索引后逐项校验完整词条，65项单测728ms通过。
- GitHub Actions 34265195144成功，单测、录音检查、构建和Pages部署均通过。
- 正式网址https://merelyfun.github.io/werewords/，Chrome390×844实测候选数6刷新保存、真实音频开局、6个不同候选、选第6词后推进并遮挡，无溢出及控制台错误。截图C:/Users/革命成功/AppData/Local/Temp/wolf-candidate-live.png已目视检查。
- 下一步：真实手机现场试玩。
